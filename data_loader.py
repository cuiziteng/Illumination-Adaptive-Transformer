...
class lowlight_loader(data.Dataset):
    ...
    def Random_Light(self, low, high):
        if random.random() > 0.5:
            pre_low = np.random.uniform(1.0, 1.25)
            low = low*pre_low
        if random.random() > 0.5:
            pre_high = np.random.uniform(0.8, 1.0)
            high = low*pre_high
        return low, high
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            data_lowlight, data_highlight = self.Random_Light(data_lowlight, data_highlight)
...
if __name__ == "__main__":
    ...
    images_path = '/home/czt/DataSets/LOL_v2/Train/Low/'
