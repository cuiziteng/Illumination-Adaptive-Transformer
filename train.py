...
def train(model, train_loader, optimizer, scheduler, config, epoch, L_content):
    # Start Training
    model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_path', type=str, default="/home/czt/DataSets/LOL_dataset/our485/low/")
    parser.add_argument('--img_val_path', type=str, default="/home/czt/DataSets/LOL_dataset/eval15/low/")
    ...
    # Data Setting
    train_dataset = lowlight_loader(images_path=config.img_path, img_size=config.img_size)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, num_workers=4,
                                               pin_memory=True)
    val_dataset = lowlight_loader(images_path=config.img_val_path, img_size=config.img_size, mode='test')
    ...
    for epoch in range(config.num_epochs):
        train(model, train_loader, optimizer, scheduler, config, epoch, L_content)
        scheduler.step()
        ssim_epoch, psnr_epoch = val(model, val_loader, epoch, ssim, psnr)

        if psnr_epoch > psnr_high:
            psnr_high = psnr_epoch
            print('the highest PSNR value is:', str(psnr_high))
            torch.save(model.state_dict(), config.snapshots_folder + "best_Epoch" + '.pth')
